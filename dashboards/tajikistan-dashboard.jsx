...
const KpiCard = ({ label, value, sub, accent = C.tj, delay = 0 }) => {
  const valColor = accent === C.tj ? C.tjL : accent === C.red ? C.redL : accent === C.gld ? C.gldL : accent === C.blu ? C.bluL : C.txt;
  return (
  ...
  return (
          ...
          React.useEffect(() => {
            ...
            async function drawMap() {
              try {
                ...
                const W=960,H=540,TJ=762;
                const proj=d3.geoMercator().center([71.0,38.8]).scale(4500).translate([W/2,H/2]);
                ...
                svg.append('g').selectAll('path').data(countries.features).join('path')
                  .attr('d',path).attr('fill',d=>+d.id===TJ?C.tj:'#1a1a1a')
                  .attr('stroke','#fff').attr('stroke-width',0.3);
                const tjF=countries.features.find(d=>+d.id===TJ);
                if(tjF) svg.append('path').datum(tjF).attr('d',path).attr('fill',C.tj).attr('stroke','#fff').attr('stroke-width',0.8);
                const labels=[
                  {name:'UZBEKISTAN',x:107,y:221},{name:'KYRGYZSTAN',x:663,y:118},
                  {name:'CHINA',x:860,y:306},{name:'AFGHANISTAN',x:455,y:429}
                ];
                ...
                svg.append('text').attr('x',ax+8).attr('y',ay+4).attr('fill','#F5C518')
                  .attr('font-size',22).attr('font-family','Inter,sans-serif').attr('font-weight',500).text('Dushanbe');
    ...
    { icon: fasvg('0 0 512 512', 'M352 256c0 22.2-1.2 43.6-3.3 64l-185.3 0c-2.2-20.4-3.3-41.8-3.3-64s1.2-43.6 3.3-64l185.3 0c2.2 20.4 3.3 41.8 3.3 64zm28.8-64l123.1 0c5.3 20.5 8.1 41.9 8.1 64s-2.8 43.5-8.1 64l-123.1 0c2.1-20.6 3.2-42 3.2-64s-1.1-43.4-3.2-64zm112.6-32l-116.7 0c-10-63.9-29.8-117.4-55.3-151.6c78.3 20.7 142 77.5 171.9 151.6zm-149.1 0l-176.6 0c6.1-36.4 15.5-68.6 27-94.7c10.5-23.6 22.2-40.7 33.5-51.5C239.4 3.2 248.7 0 256 0s16.6 3.2 27.8 13.8c11.3 10.8 23 27.9 33.5 51.5c11.6 26 20.9 58.2 27 94.7zm-209 0L18.6 160C48.6 85.9 112.2 29.1 190.6 8.4C165.1 42.6 145.3 96.1 135.3 160zM8.1 192l123.1 0c-2.1 20.6-3.2 42-3.2 64s1.1 43.4 3.2 64L8.1 320C2.8 299.5 0 278.1 0 256s2.8-43.5 8.1-64zM194.7 446.6c-11.6-26-20.9-58.2-27-94.6l176.6 0c-6.1 36.4-15.5 68.6-27 94.6c-10.5 23.6-22.2 40.7-33.5 51.5C272.6 508.8 263.3 512 256 512s-16.6-3.2-27.8-13.8c-11.3-10.8-23-27.9-33.5-51.5zM135.3 352c10 63.9 29.8 117.4 55.3 151.6C112.2 482.9 48.6 426.1 18.6 352l116.7 0zm358.1 0c-30 74.1-93.6 130.9-171.9 151.6c25.5-34.2 45.2-87.7 55.3-151.6l116.7 0z'), label: 'Area', value: '143,100 km²', note: '93% mountainous' },
            ...
            <div style={{ margin:'28px 0 8px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:3 }}>
                {tiles.map(({ icon, label, value, note }) => (
                  <div key={label} style={{ background:C.tj, border:`1px solid ${C.border}`, padding:'14px 8px 12px', position:'relative', overflow:'hidden', display:'flex', flexDirection:'column', alignItems:'center', textAlign:'center' }}>
        ...
        <div className="row g-1 mb-4">
          ...
          <div className="col-6 col-md-4 d-flex"><KpiCard label="Record High" value="48°C" sub="Southern valleys; extreme summer heat in Khatlon" accent={C.red} delay={0.10} /></div>
          <div className="col-6 col-md-4 d-flex"><KpiCard label="Record Low" value="−63°C" sub="Pamir plateau; most extreme cold in former Soviet space" accent={C.blu} delay={0.15} /></div>
        ...
        <div className="row gy-3 mb-3">
          ...
          <div className="col-12 col-md-6">
            <Panel title="Tourism Highlights" icon={Icons.landmark}>
              ...
              <GradientBar title="Tourism intensity by month (relative)" values={[8,10,18,30,50,70,100,95,90,45,15,8]} colorStops={p => `rgb(${Math.round(153+79*p/100)},${Math.round(153-128*p/100)},${Math.round(153-109*p/100)})`} unit="%" />
            </Panel>
          </div>
        </div>
        ...
        <div className="row gy-3 mb-3">
          ...
          <div className="col-12 col-md-6">
            <Panel title="Key Fiscal Indicators" icon={Icons.chart}>
              ...
              <GradientBar title="Trade balance 2015–2024 ($B)" values={[-2.3, -2.1, -2.0, -2.2, -2.2, -1.7, -2.1, 0.0, -2.3, -3.0]} xLabels={['2015', '2016', '2017', '2018', '2019', '2020', '2021', '2022', '2023', '2024']} colorStops={(p, v) => v >= 0 ? `rgb(${Math.round(153-118*p/100)},${Math.round(153+6*p/100)},${Math.round(153-89*p/100)})` : `rgb(${Math.round(153+79*p/100)},${Math.round(153-128*p/100)},${Math.round(153-109*p/100)})`} fmt={v => v > 0 ? `+${v}B` : `${v}B`} absScale={true} />
            </Panel>
          </div>
        </div>
